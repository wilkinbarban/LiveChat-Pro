...
### Requirement: kb-trainer JSON Training Workflow

(Reason: RAG ingestion replaces the CLI JSON training workflow entirely; `BOT_MODE=knowledge-base`, the fuzzy-match fallback path, `kb-trainer/`, and the build merge step are removed.)
(Migration: existing `data/knowledge-base.json` content is preserved via the one-time migration above; identity answers move to the master-prompt capability.)
