...
- [x] 4.1 RED: PR 4A coverage is complete in `tests/hygiene.test.js` and `tests/test-script.test.js`; PR 4B adds behavior-first canonical router and compatibility-alias RED coverage in `tests/admin-routes.test.js` and `tests/alias-telemetry.test.js`. Scope: all four test files. Rollback: revert the applicable tests.
...
- [x] 4.5 GREEN (PR 4B): Move canonical session, settings, and RAG route registration into substantive injected sub-router factories and add telemetry-only compatibility aliases. Finish-time logs use the actual authenticated response status; no deprecation headers are emitted before 30 representative zero-use days. Verify: `node --test tests/admin-routes.test.js tests/alias-telemetry.test.js`. Scope: `src/routes/admin*`. Rollback: revert admin router split.
