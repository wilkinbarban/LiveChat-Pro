...
|---|---:|---|
| `TELEGRAM_TOKEN` | No | Telegram bot token — bootstrap/fallback; the admin panel (Telegram tab) manages the token at runtime and takes precedence. Quoted or unquoted values are both accepted (stripped at config load) |
...
Manage your bot token in the admin panel → Telegram tab. A token saved there is
encrypted in settings and takes precedence over the `TELEGRAM_TOKEN` environment
variable, which acts as a bootstrap/fallback.
